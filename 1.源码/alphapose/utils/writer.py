...
class DataWriter():
    ...
    def is_fall(self, pose_coord_array):
        '''
        cnt = 0
        avg_eye = (pose_coord_array[0][1][1] + pose_coord_array[0][2][1]) / 2
        avg_shoulder = (pose_coord_array[0][5][1] + pose_coord_array[0][6][1]) / 2
        avg_hip = (pose_coord_array[0][11][1] + pose_coord_array[0][12][1]) / 2
        avg_knee = (pose_coord_array[0][13][1] + pose_coord_array[0][14][1]) / 2
        avg_ankle = (pose_coord_array[0][15][1] + pose_coord_array[0][16][1]) / 2
        print([avg_eye, avg_shoulder, avg_hip, avg_knee, avg_ankle])
        if (avg_ankle < avg_knee):
            cnt += 1
        if (avg_knee < avg_hip):
            cnt += 1
        if (avg_hip < avg_shoulder):
            cnt += 1
        if (avg_shoulder < avg_eye):
            cnt += 1
        if (cnt >= 2):
             return True
        '''
        ...
        while True:
            ...
            if boxes is None or len(boxes) == 0:
                ...
                for k in range(len(scores)):
                    ...
                    box_img = orig_img[int(boxes[k][1]):int(boxes[k][3]), int(boxes[k][0]):int(boxes[k][2])]
                    orig_img = orig_img.copy()

                    pose_coord_array = pose_coords[k].numpy()

                    pose_text = ['Nose', 'LEye', 'REye', 'LEar', 'REar', 'LShoulder', 'RShoulder', 'LElbow', 'RElbow', 'LWrist', 'RWrist', 'LHip', 'RHip', 'LKnee', 'Rknee', 'LAnkle', 'RAnkle']
                    for i in range(len(pose_text)):
                        #pass
                        orig_img = cv2.putText(orig_img, pose_text[i], (pose_coord_array[0][i][0], pose_coord_array[0][i][1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

                    if self.is_fall(pose_coord_array):
                        print('falling')
                        cv2.rectangle(orig_img, (int(boxes[k][0]), int(boxes[k][1])), (int(boxes[k][2]), int(boxes[k][3])), (0, 0, 255), 2)
                        fall_img = orig_img[int(boxes[k][1]):int(boxes[k][3]), int(boxes[k][0]):int(boxes[k][2])]
                        if not os.path.exists(self.opt.outputpath + '/fall'):
                            os.mkdir(self.opt.outputpath + '/fall')
                        #cv2.imshow("fall", fall_img)
                        
                        fall_img_name = os.path.join(self.opt.outputpath, 'fall', str(time.time()) + '.jpg')
                        
                        cv2.imwrite(fall_img_name, fall_img)
                    else:
                        print('not falling')
                        cv2.rectangle(orig_img, (int(boxes[k][0]), int(boxes[k][1])), (int(boxes[k][2]), int(boxes[k][3])), (0, 255, 0), 2)
                result = {
                    'imgname': im_name,
                    'result': _result
                }
