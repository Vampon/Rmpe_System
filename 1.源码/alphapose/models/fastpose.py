    ...
    def __init__(self, norm_layer=nn.BatchNorm2d, **cfg):
        ...
        if 'DCN' in cfg.keys():
            ...
            self.preact = SEResnet(
                "resnet{}".format(cfg['NUM_LAYERS']), dcn=dcn, stage_with_dcn=stage_with_dcn)
        else:
            self.preact = SEResnet("resnet{}".format(cfg['NUM_LAYERS']))

        # Imagenet pretrain model
        import torchvision.models as tm   # noqa: F401,F403
        assert cfg['NUM_LAYERS'] in [18, 34, 50, 101, 152]
        x = eval("tm.resnet{}(pretrained=True)".format(cfg['NUM_LAYERS']))
