...
""" 参数 """
parser = argparse.ArgumentParser(description='Rmpe_System')
...
def kk(image_cvs, win, det_loader):
    while 1:
        (inps, frame, im_name, boxes, scores, ids, cropped_boxes) = det_loader.read()
        #ret, frame = det_loader.read()
        #if ret == True:
        if 1:
...
def loop(image_cvs, win, det_loader):
    n = 0
    while True:
        yield n
        n += 1
...
def video_show(image_cvs, win, det_loader):
    im_names_desc = tqdm(loop(image_cvs, win, det_loader))
    return im_names_desc
def cc(det_loader):
    win = tk.Tk()
    win.title("多目标异常行为检测系统")
    win.geometry('960x720')                 
    win.resizable(width=False, height=True) 
    image_cvs = tk.Canvas(win, width=640, height=480, bg='white')
    image_cvs.grid(row = 0, column = 0)
    im_names_desc = tqdm(loop(image_cvs, win, det_loader))
    #t = threading.Thread(target=kk, args=(image_cvs, win, det_loader))
    #t.setDaemaon(True)
    #t.start()
    return im_names_desc
...
if __name__ == "__main__":
    ...
    print('Loading pose model from {}...'.format(args.checkpoint))
